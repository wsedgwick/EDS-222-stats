...
**Possum tail length and total length appear to be moderately correlated. There will be a positive correlation. Possum tail length and head length do not seem to be correlated, although there is a slight positive trend. There will be a positive correlation.**
...
\*\*My estimate was very close to the correlation coefficient between tail length and total length, but my estimate was 0.15 off from the correlation coefficient between tail length and head length. There was more correlation between tail length and head length than I estimated.
...
# exploratory code
ggplot(data = possum, aes(x = skull_w, head_l)) +
  geom_point() +
...
**The model suggests there is a relationship between tail length and body length. When the tail length is from 32 cm to 43 cm, we can roughly predict the total length of possums. The 5 number summary table shows that the residuals are not overly skewed.**
...
# Generally, this graph is roughly normal
...
\*\*The mean of 4.91 × 10\^[^14]) is basically 0.

[^6]: **Hint:** In `dplyr` you can use `add_predictions(mod)` where `mod` is a regression model to generate a new variable containing your model's predicted values.
...
**The residuals look approximately constant across all values, and appear to have an even variance over the tail length.**
